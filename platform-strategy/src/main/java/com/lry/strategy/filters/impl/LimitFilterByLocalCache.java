...
import com.lry.strategy.cache.LimitLocalCache;
 ...
 */
@Component
public class LimitFilterByLocalCache implements FilterChain {
    ...
    public boolean dealSms(Standard_Submit standard_submit) {
        ...
        //先获取限流的策略
        Set<Map> allLimits = limitLocalCache.getAllLimits();
        long max = System.currentTimeMillis();//以当前为基准
        //哪个每一个策略进行判断
        for (Map allLimit : allLimits) {
            Integer count = 0;
            try {
                String value = (String) allLimit.get("value");//限制的次数
                count = Integer.parseInt(value);//转成数字
            } catch (Exception e) {
                e.printStackTrace();
            }
